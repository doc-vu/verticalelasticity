...
import scipy.stats
import logging
from SHS_model_v2 import SHS_model
...
class CloudModel(object):
        ...
        @abstractmethod
        def sysControl(self, req_rate, target_stats, latency, host_stats, current_cpus, MaxN, MinN):
		return
...
class SHS_model_old:
	...
	self.latencyModel = create_cloud_model('THRESHOLD_LATENCY')
...
#        return Y
    ...
    def sysControl(self, sysIn, sysStat, sysOut,sysClus, controlFeedback, MaxN, MinN):
        ...
        if self.verbose:
	    print('step: ' + str(len(self.__controlOut)), file = log )
            print('predicted latency: N(' +str(ym)  +','+str(ysi) +')', file = log )
            print('P(L > '+ str(self.__alpha) +')= ' + str(p_lge_alpha), file = log )
            print('P(L < ' + str(self.__beta) + ')= ' + str(P_lle_beta), file = log )
        ...
        if p_lge_alpha >= self.__sigma and self.__controllerCounter >= self.__controllerDelay and self.__controlFeedback < MaxN:
            ...
            if self.verbose:
	            print('P(L > '+ str(self.__alpha2) +')= ' + str(p_lge_alpha2), file = log )
            ...
            if self.verbose:
                print('P(L > '+ str(self.__beta2) +')= ' + str(P_lle_beta2), file = log )
	...
	# adjust the control signal for current value
	cpus = self.latencyModel.sysControl(0, None, sysOut,None,
                                                controlFeedback, 0,0)
	if cpus > controlSignal:
		return cpus
    ...
    def export_results(self):
        ...
        # time
        l1 = self.__out_pred_data[:n]
        l2 = self.__out_mean[:n]
        l3 = lowerBound
        l4 = upperBound

        l5 = self.__In_pred_data
        l6 = self.__In_mean[:n]
        l7 = lowerBoundIn
        l8 = upperBoundIn
...
class ThresholdModel(CloudModel):
	...
	def __init__(self):
		...
		#cpu_util = target_stats[0]/current_cpus
		cpu_util = target_stats[0]
		print ("CPU Util: " + str(cpu_util))
		# invalid 
		if cpu_util < 0.0:
			return current_cpus
		if cpu_util > 80:
			return current_cpus + 2
		elif cpu_util > 70:
			return current_cpus + 1
		elif cpu_util < 30:
			return current_cpus - 2
		elif cpu_util < 50:
			return current_cpus - 1
...
class ThresholdLatencyModel(CloudModel):
...
def create_cloud_model(model_type):
#        if model_type == 'GAUSSIAN':
#                return GaussianModel()
        if model_type == 'THRESHOLD':
                return ThresholdModel()
        elif model_type == 'THRESHOLD_LATENCY':
                return ThresholdLatencyModel()
        elif model_type == 'SHS':
		latmodel = ThresholdLatencyModel()
                return SHS_model(alpha=0.12, beta=0.08, alpha2=0.2, beta2=0.06, sigma = 0.5, latModel=latmodel)

        else:
                raise NotImplementedError("Cloud model type has not been implemented...")
