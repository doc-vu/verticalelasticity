...
class SHS_model:
    ...
    def __init__(self, winSize=350, gpWinSize=200,gpin_winSize=150 ,nu_cluster=2,
                 controlFeedback=1, alpha=3, beta=2, sigma=0.9, alpha2=4, beta2=1.5,controllerDelay=1, latModel=None):

	self.latencyModel = latModel 
        #  online Learning parameters and variables
        self.__winSize = winSize
        self.__gpWinSize = gpWinSize
        self.__gpin_winSize = gpin_winSize 
        self.__fileName = "data_init_4cores.txt"
        # system data
        self.__inData = []
        self.__outData = []
        self.__statData = []
        ...
        indxS = [14]     # State indices
        indxQ = [2, 4, 10, 13]  #clustering data indicies
    ...
    def __initModel(self, sysIn, sysStat, sysOut, sysClus):
        ...
        if len(self.__inData) >= self.__winSize:
            # Learn the models (align the data to input output format Y(k+1) =
            # f(x(k),u(k+1)))
            self.__inData.pop(0)
    ...
    def __updateModel(self, sysIn, sysStat, sysOut,sysClus):
        ...
        try:
            ...
            print('WARNING: Update forecast model FAILED')
            #self.__gpFmdl = gp.GPR()
            #k_f = gp.cov.RBF(log_ell=hyp_f[0], log_sigma=hyp_f[1])
            #self.__gpFmdl.setPrior(mean=gp.mean.Zero(), kernel=k_f)
            #self.__gpFmdl.setNoise(log_sigma=np.log(0.8))
        ...
        gprMdl = gp.GPR()
        m = gp.mean.Zero()
        # k = gp.cov.SumOfKernel(gp.cov.RBFard(D=None, log_ell_list=hyp,
        # log_sigma=1.),gp.cov.Noise(1))
        ...
        try:
            ...
            print('WARNING: Update system model Failed')
            #gprMdl = gp.GPR()
            #m = gp.mean.Zero()
            # k = gp.cov.SumOfKernel(gp.cov.RBFard(D=None, log_ell_list=hyp,
            # log_sigma=1.),gp.cov.Noise(1))
    ...
    def __predictPerformance(self):
        ...
        INm, INsi, foom, foos2, foolp = self.__gpFmdl.predict(np.array([self.__modelTimeIdx]).reshape(1, -1))
    ...
    def plot_results(self):
        ...
        ax1.fill(np.concatenate([x, x[::-1]]),
                 ...
                 #       alpha=.5, fc='b', ec='b', label='95% confidence
                 #       interval')
